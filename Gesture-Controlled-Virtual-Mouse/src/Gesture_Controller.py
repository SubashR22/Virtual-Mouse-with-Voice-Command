...
class Controller: 
    ...
    def changesystembrightness():
        try:
            ...
            print(f"Brightness values: {brightness_values}")  # debug
        
        # Handle both list and single value cases
            if isinstance(brightness_values, list):
                current_brightness = brightness_values[0]
            else:
                current_brightness = brightness_values
            
        # Convert to 0-1 scale
            currentBrightnessLv = current_brightness / 100.0
        ...
        with mp_hands.Hands(max_num_hands=2, min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands: 
            while GestureController.cap.isOpened() and GestureController.gc_mode: 
                ...
                if results.multi_hand_landmarks:                         
                    ...
                    # Handle major hand gestures (right hand by default) 
                    if gest_name_major != Gest.PALM: 
                        Controller.handle_controls(gest_name_major, handmajor.hand_result) 

                    # Handle minor hand gestures (left hand by default) 
                    if gest_name_minor != Gest.PALM: 
